...
import { useCheckinStats, useGenerateCheckinQr, useManualCheckin, useScanCheckin, useSessionCheckins } from "@/hooks/use-checkin"
...
export default function CheckinPage() {
  ...
  const { data: sessionCheckins, isLoading: checkinsLoading } = useSessionCheckins(sessionId || undefined)
  const { data: stats } = useCheckinStats(courseId || undefined)
  const generateQr = useGenerateCheckinQr()
      ...
      <Card className="border-border/70 bg-card/90 shadow-[0_18px_50px_rgba(11,42,85,0.08)]">
        <CardContent className="flex flex-col gap-5 p-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl space-y-3">
            ...
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-balance">Check-in</h1>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Genera QR, registra asistencia y revisa estadísticas por curso o sesión.
              </p>
            </div>
          </div>
            ...
            {stats ? (
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded-2xl border border-border/70 bg-muted/30 p-3">
                    <div className="text-xs uppercase text-muted-foreground">Check-ins</div>
      ...
      <Card className="border-border/70 bg-card/90 shadow-[0_18px_50px_rgba(11,42,85,0.08)]">
        <CardContent className="p-2 sm:p-4">
          <DataTable
            ...
            emptyMessage="No hay check-ins para esa sesión"
          />
        </CardContent>
      </Card>
    </div>
